...
class DataType:
    ...
    BOOL16 = "bool16"             # 布尔值，占1个寄存器（256=true，0=false）

    REGISTER_COUNT = {            # 每种类型占用的寄存器数量映射表
        ...
        "bool16": 1,              # bool16 占 1 个寄存器（读 WORD 判断 256/0）
    }


# ======================== 寄存器 → 值 =========================
def register_to_int16(value: int) -> int:
    """无符号寄存器值 → 有符号16位整数（范围 -32768~32767）"""
...
def register_to_bool16(value: int) -> bool:
    ...
    return value == 256           # 值等于256表示true，否则为false
...
def convert_registers(registers: list, data_type: str):
    ...
    """
    if data_type == DataType.INT16:                   # 如果是 int16 类型
        return register_to_int16(registers[0])        # 取第一个寄存器转换
    elif data_type == DataType.UINT16:                # 如果是 uint16 类型
        return register_to_uint16(registers[0])       # 取第一个寄存器转换
    elif data_type == DataType.INT32:                 # 如果是 int32 类型
        return registers_to_int32(registers)          # 取两个寄存器转换
    elif data_type == DataType.UINT32:                # 如果是 uint32 类型
        return registers_to_uint32(registers)         # 取两个寄存器转换
    elif data_type == DataType.FLOAT32:               # 如果是 float32 类型
        return registers_to_float(registers)          # 取两个寄存器转换
    elif data_type == DataType.BOOL16:                # 如果是 bool16 类型
        return register_to_bool16(registers[0])       # 取第一个寄存器，判断是否等于256
    else:                                             # 未知类型
