"""
CSV 文件管理模块
负责创建、写入、轮转和清理 CSV 数据日志文件
"""

import csv                                                     # CSV 读写模块
from datetime import datetime                                  # 时间模块，用于生成文件名时间戳
from pathlib import Path                                       # 路径模块，用于文件操作
...
def build_headers(reg_map: dict) -> list:
    ...
    for addr in sorted(reg_map.keys()):                        # 按地址顺序遍历所有变量
        _, name, desc, *_ = reg_map[addr]                      # 解包获取变量名和描述
        headers.append(f"{name}({desc})" if desc else name)    # 有描述则拼接，否则只用变量名
    return headers                                             # 返回完整表头列表


def build_row(timestamp: str, values: dict,
              reg_map: dict, fmt_func) -> list:
    """
    构建一行 CSV 数据
    ...
    :param fmt_func:  格式化函数 (dtype, val) -> str
    :return: 一行数据列表
    """
...
def create_csv(reg_map: dict):
    ...
    :param reg_map: 变量映射字典
    :return: (文件对象, csv writer 对象, 文件名) 元组
    """
    # 生成带时间戳的文件名
    filename = f"data_log_{datetime.now():%Y%m%d_%H%M%S}.csv"

    # 创建文件（UTF-8 BOM 编码，Excel 兼容）
